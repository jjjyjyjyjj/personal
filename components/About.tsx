    ...
    <section id="about" className="about-grid" style={{ backgroundColor:T.ivory, display:"grid", gridTemplateColumns:"1fr 1fr", minHeight:"100vh" }}>
      ...
      <div className="about-image-col" style={{ background:T.parchment, position:"relative", overflow:"hidden", minHeight:"100vh" }}>
        <div ref={scalesRef} className="parallax-wrap" style={{ position:"absolute",  height:"100%" }}>
          <img src="/about.png" alt="artwork" style={{  width:"110%", height:"100%", objectFit:"cover", objectPosition:"center" }} />
          {/* <AboutPetals /> */}
          </div>
      </div>
      ...
      <div className="reveal about-text-col" style={{ paddingLeft:"5rem", paddingRight:"5rem", paddingTop:"100px", display:"flex", flexDirection:"column", justifyContent:"center", position:"relative", minHeight:"100vh" }}>
        <div ref={textRef} className="parallax-wrap">
          ...
          <h2 style={{ fontFamily:"'Helvetica Neue', sans-serif", fontSize:"clamp(2rem,3.5vw,3.2rem)", fontWeight:900, letterSpacing:"-0.08em", lineHeight:1.2, marginBottom:"2rem", color:T.charcoal }}>Who am I?</h2>
          <p style={{ fontFamily:"'Helvetica Neue', sans-serif", fontSize:"1.05rem", fontWeight:300, lineHeight:1.9, color:T.greyDark, maxWidth:"34rem", marginBottom:"3rem" }}>
          ...
          <div style={{ display:"flex", gap:0, marginBottom:"3rem", borderTop:`1px solid ${T.parchment}`, borderBottom:`1px solid ${T.parchment}`, padding:"2rem"}}>
            {["Cybersecurity","Low-level systems","Ugly animals"].map((item, i, arr) => (
              <div key={item} style={{ paddingRight: i < arr.length-1 ? "2rem" : 0, borderRight: i < arr.length-1 ? `1px solid ${T.parchment}` : "none", marginRight: i < arr.length-1 ? "2rem" : 0, marginBottom:"0.5rem" }}>
                <div style={{ fontFamily:"'Helvetica Neue', sans-serif", fontSize:"1rem", fontStyle:"italic", color:T.charcoal }}>{item}</div>
                <div style={{ fontFamily:"'Helvetica Neue', sans-serif", fontSize:"0.45rem", letterSpacing:"0.3em", textTransform:"uppercase", color:T.grey, marginTop:"0.2rem" }}>interest</div>
              </div>
            ))}
          </div>
        </div>
